...
export async function generateMetadata({ params }: PageProps): Promise<Metadata> {
  ...
  if (!item) {
    return { title: 'Service Not Found | Auto Repair' }
  }
                ...
                <dl className="space-y-4">
                  ...
                  {(item as any).priceRange && (
                    <div>
                      ...
                      <dd className="font-semibold text-gray-900">{(item as any).priceRange}</dd>
                    </div>
                  )}
                  {(item as any).estimatedTime && (
                    <div>
                      <dt className="text-sm text-gray-500">Estimated Time</dt>
                      <dd className="font-semibold text-gray-900">{(item as any).estimatedTime}</dd>
                    </div>
                  )}
                  {(item as any).warrantyInfo && (
                    <div>
                      <dt className="text-sm text-gray-500">Warranty Information</dt>
                      <dd className="font-semibold text-gray-900">{(item as any).warrantyInfo}</dd>
                    </div>
                  )}
                </dl>
                <div className="mt-8">
