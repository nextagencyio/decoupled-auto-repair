...
import { GET_COUPON_BY_PATH } from '@/lib/queries'
import { DrupalCoupon } from '@/lib/types'
...
interface CouponByPathData {
  route: {
    entity: DrupalCoupon
  } | null
}

async function getCoupon(path: string): Promise<DrupalCoupon | null> {
  try {
    const requestHeaders = await headers()
    const apolloClient = getServerApolloClient(requestHeaders)
    const { data } = await apolloClient.query<CouponByPathData>({
      query: GET_COUPON_BY_PATH,
      variables: { path },
...
export async function generateMetadata({ params }: PageProps): Promise<Metadata> {
  ...
  const path = `/coupons/${slug.join('/')}`
  const item = await getCoupon(path)

  if (!item) {
    return { title: 'Coupon Not Found | Auto Repair' }
  }

  return {
    title: `${item.title} | Auto Repair`,
              ...
              <div className="bg-white rounded-xl shadow-sm p-6 sticky top-24">
                ...
                <dl className="space-y-4">
                  {(item as any).discountAmount && (
                    <div>
                      <dt className="text-sm text-gray-500">Discount Amount</dt>
                      <dd className="font-semibold text-gray-900">{(item as any).discountAmount}</dd>
                    </div>
                  )}
                  {(item as any).couponCode && (
                    <div>
                      <dt className="text-sm text-gray-500">Coupon Code</dt>
                      <dd className="font-semibold text-gray-900">{(item as any).couponCode}</dd>
                    </div>
                  )}
                </dl>
                <div className="mt-8">
